...
export default function MeetingDetail() {
  ...
  const [markdownBusy, setMarkdownBusy] = useState<boolean>(false)
  ...
  const googleSyncUiEnabled =
    import.meta.env.VITE_ENABLE_GOOGLE_LOGIN === 'true' ||
    import.meta.env.VITE_ENABLE_GOOGLE_SYNC === 'true'

  const handleGoogleSyncToggle = async (next: boolean) => {
    if (!googleSyncUiEnabled || !id) return
    setGoogleSyncLoading(true)
    try {
      const res = await fetch(api(`/api/events/${id}/google-sync`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      if (!res.ok) {
        ...
        throw new Error(text || 'Google同期に失敗しました')
      }
      const data = await res.json()
      setGoogleSyncEnabled(Boolean(data?.google_sync_enabled))
      setInfo((prev: any) =>
        prev ? { ...prev, google_sync_enabled: data?.google_sync_enabled, google_event_id: data?.google_event_id } : prev
      )
    } catch (err) {
      alert(err instanceof Error ? err.message : 'Google同期に失敗しました')
    } finally {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setGoogleSyncEnabled(Boolean(j.event?.google_sync_enabled))
  ...
  const reloadEvent = async () => {
    try {
      const res = await fetch(api(`/api/events/${id}`), { cache: 'no-store' as RequestCache, credentials: 'include' })
      const j = await res.json()
      setInfo(j.event)
      setSegments(j.segments || [])
      setGoogleSyncEnabled(Boolean(j.event?.google_sync_enabled))
  ...
  const formatDateTime = (ts?: number | null) => (typeof ts === 'number' && ts > 0 ? new Date(ts * 1000).toLocaleString() : '-')
  const formatArtifactTime = (ts?: number | null) => (typeof ts === 'number' && ts > 0 ? new Date(ts * 1000).toLocaleString() : '-')
  ...
  const triggerMarkdownDownload = (filename: string, text: string) => {
  ...
  const sanitizeText = (text?: string | null) => (text ? text.replace(/\r\n/g, '\n').trim() : '')

  const downloadMeetingMarkdown = async () => {
    if (!id || markdownBusy) return
    setMarkdownBusy(true)
    try {
      const [eventRes, minutesRes, artifactsRes] = await Promise.all([
        fetch(api(`/api/events/${id}`), { cache: 'no-store' as RequestCache, credentials: 'include' }),
        fetch(api(`/api/events/${id}/minutes`), { cache: 'no-store' as RequestCache, credentials: 'include' }),
        fetch(api(`/api/events/${id}/artifacts`), { cache: 'no-store' as RequestCache, credentials: 'include' }),
      ])
      if (!eventRes.ok) throw new Error('EVENT_FETCH_FAILED')
      const eventPayload = await eventRes.json()
      const eventInfo = eventPayload.event || {}
      const segs: any[] = Array.isArray(eventPayload.segments) ? eventPayload.segments : []
      const summaryText = sanitizeText(eventPayload.summary?.text_md)
      let minutesText = ''
      if (minutesRes.ok) {
        const minutesJson = await minutesRes.json()
        minutesText = sanitizeText(minutesJson.md)
      }
      let artifactList: any[] = artifacts
      if (artifactsRes.ok) {
        const artifactJson = await artifactsRes.json()
        if (Array.isArray(artifactJson.items)) {
          artifactList = artifactJson.items
        }
      }
      const parsedParticipants = (() => {
        try {
          if (eventInfo.participants_json) {
            return JSON.parse(eventInfo.participants_json)
          }
        } catch {}
        return { self: participants.self, client: participants.client }
      })()
      const selfMembers: string[] = Array.isArray(parsedParticipants?.self) ? parsedParticipants.self : []
      const clientMembers: string[] = Array.isArray(parsedParticipants?.client) ? parsedParticipants.client : []
      const describeMembers = (items: string[]) => (items.length ? items.join(', ') : '（未入力）')
      const now = new Date()
      const fileStamp = `${now.getFullYear()}${z(now.getMonth()+1)}${z(now.getDate())}_${z(now.getHours())}${z(now.getMinutes())}${z(now.getSeconds())}`
      const headerTitle = eventInfo.title || info?.title || '無題の会議'
      const mdParts: string[] = []
      mdParts.push(`# ${headerTitle}`)
      mdParts.push('')
      mdParts.push(`- 会議ID: \`${id}\``)
      mdParts.push(`- エクスポート日時: ${now.toLocaleString()}`)
      mdParts.push(`- 開催日時: ${formatDateTime(eventInfo.start_ts)}`)
      mdParts.push(`- 言語: ${eventInfo.lang || '-'}`)
      if (eventInfo.translate_to) mdParts.push(`- 翻訳: ${eventInfo.translate_to}`)
      mdParts.push('')
      mdParts.push('## 参加者')
      mdParts.push('')
      mdParts.push(`- 自社: ${describeMembers(selfMembers)}`)
      mdParts.push(`- 先方: ${describeMembers(clientMembers)}`)
      mdParts.push('')
      mdParts.push('## 要約')
      mdParts.push('')
      mdParts.push(summaryText || 'まだ要約はありません。（「生成」を押して作成してください）')
      mdParts.push('')
      mdParts.push('## 議事録（エディタ）')
      mdParts.push('')
      mdParts.push(minutesText || '議事録はまだ保存されていません。')
      mdParts.push('')
      mdParts.push('## アーティファクト')
      mdParts.push('')
      if (artifactList.length === 0) {
        mdParts.push('- ファイルはまだありません')
      } else {
        artifactList.forEach((a: any) => {
          const link = resolveDownloadUrl(a.url || '')
          mdParts.push(`- ${a.name} (${formatBytes(a.size)} / 更新: ${formatArtifactTime(a.mtime)}) ${link}`)
        })
      }
      mdParts.push('')
      mdParts.push('## ライブ字幕')
      mdParts.push('')
      if (segs.length === 0) {
        mdParts.push('- 収録済みの字幕はありません。')
      } else {
        segs.forEach((seg: any) => {
          const text = sanitizeText(seg.text_ja || seg.text || seg.text_mt)
          mdParts.push(`- [${formatClock(seg.start)} - ${formatClock(seg.end ?? seg.start)}] ${seg.speaker || 'S?'}: ${text || '(テキストなし)'}`)
        })
      }
      mdParts.push('')
      mdParts.push('---')
      mdParts.push('_このMarkdownは Local Minutes から自動生成されました_')
      const filename = `meeting_${id}_${fileStamp}.md`
      triggerMarkdownDownload(filename, mdParts.join('\n'))
      dlog(`markdown export done -> ${filename}`)
    } catch (err) {
      console.error('markdown export failed', err)
      alert('Markdownの生成に失敗しました。時間をおいて再度お試しください。')
    } finally {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        ...
        if (!r.ok) {
          dlog(`artifacts fetch failed status=${r.status}`)
          if (r.status === 404) {
            stop = true
            setLoadError('この会議は存在しません。再読み込みして新規作成してください。')
          }
          return
        }
        ...
        dlog('artifacts fetch error')
        if (debug) console.warn('artifacts fetch failed', err)
      } finally {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key.toLowerCase() === 'r') {
        ...
        fetch(api(`/api/events/${id}/postprocess`), { method: 'POST', credentials: 'include' }).catch(() => {})
      }
    }
  ...
  const start = async () => {
    ...
    const res = await fetch(api(`/api/events/${id}/start`), { method: 'POST', credentials: 'include' })
    const j = await res.json()
    const t = j.token
    setToken(t)
    ...
    ws.onmessage = (ev) => {
      ...
      if (msg.type === 'partial') {
        ...
        setSegments(prev => {
          if (typeof msg.rowId === 'number' && prev.some((s: any) => s.rowId === msg.rowId)) {
            return prev
          }
          return [...prev, { start: msg.range[0], end: msg.range[1], speaker: msg.speaker, text_ja: msg.text, text_mt: msg.mt, rowId: msg.rowId }]
        })
        dlog(`final segment s=${msg.range?.[0]} e=${msg.range?.[1]} text='${msg.text}'`)
        // 確定したら partial はクリア
        setPartialText("")
      } else if (msg.type === 'final-update') {
        setSegments(prev =>
          prev.map((seg: any) =>
            seg.rowId === msg.rowId
              ? { ...seg, start: msg.range?.[0] ?? seg.start, end: msg.range?.[1] ?? seg.end, speaker: msg.speaker ?? seg.speaker, text_ja: msg.text ?? seg.text_ja, text_mt: msg.mt ?? seg.text_mt }
              : seg
          )
        )
  ...
  const stop = async () => {
    ...
    await fetch(api(`/api/events/${id}/stop`), { method: 'POST', credentials: 'include' }).catch(()=>{})
    ...
    try {
      ...
      const r = await fetch(api(`/api/events/${id}/artifacts`), { credentials: 'include' })
      const j = await r.json()
      setArtifacts(j.items || [])
      ...
      <div className="card" onContextMenu={remove} title="右クリックで削除">
        {loadError && (
          <div className="mb-3 rounded-lg border border-red-200 bg-red-50 text-red-700 p-3 text-sm">
            ...
            <div className="mb-2">3秒後に会議一覧へ自動的に戻ります。</div>
            <button
              ...
              onClick={async () => {
                try {
                  ...
                  const res = await fetch(api('/api/events'), {
                    ...
                    headers: { 'Content-Type': 'application/json' },
                    credentials: 'include',
                    body: JSON.stringify({ title: '新規会議', start_ts: now, lang: 'ja' })
                  })
                  const j = await res.json()
        ...
        <div className="shrink-0 flex items-center gap-2">
          <button
            ...
            onClick={downloadMeetingMarkdown}
            disabled={markdownBusy}
          >
            {markdownBusy ? '生成中…' : 'Markdownダウンロード'}
          </button>
          {googleSyncUiEnabled && (
            <button
              className={`btn ${googleSyncEnabled ? 'btn-secondary' : 'btn-ghost'} whitespace-nowrap`}
              disabled={googleSyncLoading}
              onClick={() => handleGoogleSyncToggle(!googleSyncEnabled)}
            >
              {googleSyncLoading
                ? '同期中…'
                : googleSyncEnabled
                ...
                <div>
                  <button className="btn btn-ghost text-xs" onClick={async()=>{
                    ...
                    await fetch(api(`/api/events/${id}`), {
                      ...
                      headers: { 'Content-Type': 'application/json' },
                      credentials: 'include',
                      body: JSON.stringify({ participants_json: pj })
                    })
                  }}>参加者を保存</button>
                </div>
              </div>
