...
export default function Calendar() {
  ...
  const googleSyncUiEnabled = import.meta.env.VITE_ENABLE_GOOGLE_LOGIN === 'true' || import.meta.env.VITE_ENABLE_GOOGLE_SYNC === 'true'
  const [monthSyncChecked, setMonthSyncChecked] = useState(false)
  const [monthSyncLoading, setMonthSyncLoading] = useState(false)
  ...
  const create = async (withRecord = false) => {
    ...
    setOpen(false)
    navigateApp(withRecord ? `/recording?event_id=${j.id}&autostart=1` : `/meetings/${j.id}`)
  }

  const toggleMonthSync = async (next: boolean) => {
    if (!googleSyncUiEnabled) return
    setMonthSyncLoading(true)
    try {
      const res = await fetch(api('/api/events/calendar/google-sync'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ year, month: month + 1, enabled: next }),
      })
      if (!res.ok) {
        const text = await res.text()
        throw new Error(text || 'Google同期に失敗しました')
      }
      setMonthSyncChecked(next)
      await load()
    } catch (err) {
      alert(err instanceof Error ? err.message : 'Google同期に失敗しました')
    } finally {
      setMonthSyncLoading(false)
    }
  }

  const onOpen = (id: string) => { navigateApp(`/meetings/${id}`) }
        ...
        <div className="flex items-center gap-2">
          {googleSyncUiEnabled && (
            <label className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300">
              <input
                type="checkbox"
                checked={monthSyncChecked}
                disabled={monthSyncLoading}
                onChange={(e) => toggleMonthSync(e.target.checked)}
              />
              今月をGoogleと同期
              {monthSyncLoading && <span>同期中…</span>}
            </label>
          )}
